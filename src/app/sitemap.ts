import { MetadataRoute } from 'next'

export default function sitemap(): MetadataRoute.Sitemap {
  return [
    {
      url: 'https://www.weltongolf.com',
      lastModified: new Date('2024-12-20'),
    },
    {
      url: 'https://www.weltongolf.com/course-directory',
      lastModified: new Date('2024-12-18'),
    },
    {
      url: 'https://www.weltongolf.com/tools',
      lastModified: new Date('2024-12-19'),
    },
    // Golf Travel Articles
    {
      url: 'https://www.weltongolf.com/travel/best-golf-breaks-uk',
      lastModified: new Date('2024-12-15'),
    },
    {
      url: 'https://www.weltongolf.com/travel/best-golf-breaks-wales',
      lastModified: new Date('2024-12-12'),
    },
    {
      url: 'https://www.weltongolf.com/travel/best-golf-breaks-bournemouth',
      lastModified: new Date('2024-12-10'),
    },
    // Golf Improvement Articles
    {
      url: 'https://www.weltongolf.com/break-80',
      lastModified: new Date('2026-01-14'),
    },
    {
      url: 'https://www.weltongolf.com/break-90',
      lastModified: new Date('2026-01-14'),
    },
    {
      url: 'https://www.weltongolf.com/break-100',
      lastModified: new Date('2026-01-14'),
    },
    {
      url: 'https://www.weltongolf.com/scoring',
      lastModified: new Date('2026-01-14'),
    },
    {
      url: 'https://www.weltongolf.com/scoring/how-golf-scoring-works',
      lastModified: new Date('2026-01-14'),
    },
    {
      url: 'https://www.weltongolf.com/travel',
      lastModified: new Date('2026-01-14'),
    },
    // Handicap & Scoring Calculators
    {
      url: 'https://www.weltongolf.com/tools/handicap-calculator',
      lastModified: new Date('2024-12-16'),
    },
    {
      url: 'https://www.weltongolf.com/tools/course-handicap-calculator',
      lastModified: new Date('2024-12-14'),
    },
    {
      url: 'https://www.weltongolf.com/tools/stableford-calculator',
      lastModified: new Date('2024-12-11'),
    },
    {
      url: 'https://www.weltongolf.com/tools/strokes-gained-calculator',
      lastModified: new Date('2024-12-09'),
    },
    // Distance & Performance Calculators
    {
      url: 'https://www.weltongolf.com/tools/ball-speed-calculator',
      lastModified: new Date('2024-12-13'),
    },
    {
      url: 'https://www.weltongolf.com/tools/swing-speed-calculator',
      lastModified: new Date('2024-12-07'),
    },
    {
      url: 'https://www.weltongolf.com/tools/club-distance-calculator',
      lastModified: new Date('2024-12-06'),
    },
    {
      url: 'https://www.weltongolf.com/tools/range-ball-distance-calculator',
      lastModified: new Date('2024-12-04'),
    },
    {
      url: 'https://www.weltongolf.com/tools/altitude-distance-calculator',
      lastModified: new Date('2024-12-02'),
    },
    {
      url: 'https://www.weltongolf.com/tools/wind-elevation-adjuster',
      lastModified: new Date('2024-11-30'),
    },
    // Equipment & Fitting Calculators
    {
      url: 'https://www.weltongolf.com/tools/club-fitting-estimator',
      lastModified: new Date('2024-11-28'),
    },
    {
      url: 'https://www.weltongolf.com/tools/club-length-calculator',
      lastModified: new Date('2024-11-26'),
    },
    {
      url: 'https://www.weltongolf.com/tools/grip-size-calculator',
      lastModified: new Date('2024-11-24'),
    },
    {
      url: 'https://www.weltongolf.com/tools/shaft-flex-calculator',
      lastModified: new Date('2024-11-22'),
    },
    // Course & Planning Tools
    {
      url: 'https://www.weltongolf.com/tools/tee-recommendation-calculator',
      lastModified: new Date('2024-11-20'),
    },
    {
      url: 'https://www.weltongolf.com/tools/playing-time-estimator',
      lastModified: new Date('2024-11-18'),
    },
    {
      url: 'https://www.weltongolf.com/tools/golf-trip-planner',
      lastModified: new Date('2024-11-25'),
    },
    // Legal Pages
    {
      url: 'https://www.weltongolf.com/privacy-policy',
      lastModified: new Date('2024-11-15'),
    },
    {
      url: 'https://www.weltongolf.com/terms-of-service',
      lastModified: new Date('2024-11-15'),
    },
  ]
}